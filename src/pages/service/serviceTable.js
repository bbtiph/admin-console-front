...
const NAME_OF_SERVICE = "NAME_OF_SERVICE";
const TYPE = "TYPE";
...
export default function ServiceTable(props) {
  ...
  let pagesCount = Math.ceil(props.servicePage.totalCount / 10);
  let pages = [];
  ...
  const setIsOnline = (e, service) => {
    service.onlinee = e.target.value == "-" ? null : e.target.value;
    saveService(service)
  }

  const setIsEnabled = (e, service) => {
    service.enabledd = e.target.value == "-" ? null : e.target.value;
    saveService(service)
  }
  ...
  const saveService = (service) => {
    ...
    axios.post(`/admin-console-app/crm-server/v2/save-service`, service, config).then(
      res => {
    ...
    ).catch(err => console.log("kjjk>> ", err));
  }

  let isListOfServices = props.servicePage.serviceList;
  return (
    <>
      <Component />
        ...
        <div className="serviceTable">
          <div className="upperTableToDo">
            ...
            <div className="toDoButton">
              <button className="btn btn-primary"><svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="table-box">
            <table className="table">
              ...
              <tbody>
                {
                  isListOfServices.map((service, i) => (
                    <tr>
                      <td><a style={{color: '#4184ce'}}>{service.contractorName}</a></td>
                      <td>
                        <select className="form-control" style={{ border: 'none' }} onChange={(e) => setIsOnline(e, service)}>
                          <option value={null} selected={service.onlinee == null ?? "selected"}>-</option>
                          <option value={true} selected={service.onlinee == true ?? "selected"}>Online</option>
                          <option value={false} selected={service.onlinee == false ?? "selected"}>Offline</option>
                        </select>
                      </td>
                      <td>
                        <select className="form-control" style={{ border: 'none' }} onChange={(e) => setIsEnabled(e, service)}>
                          <option value={null} selected={service.enabledd == null ?? "selected"}>-</option>
                          <option value={true} selected={service.enabledd == true ?? "selected"}>Да</option>
                          <option value={false} selected={service.enabledd == false ?? "selected"}>Нет</option>
                        </select>
                      </td>
                      <td>{service.date}</td>
                    </tr>
            ...
            <nav>
              <ul className="pagination justify-content-end">
                <li className="page-counter"><a className="page-link">
                  {(props.servicePage.currentPage - 1) * 10 + 1}-{(props.servicePage.currentPage) * 10} их {pagesCount} страниц</a>
                </li>
                <li onClick={() => {
                  props.onChangePage({
                    pageNumber: props.servicePage.currentPage - (props.servicePage.currentPage - 1 >= 1 ? 1 : 0),
                ...
                {
                  pages.slice(props.servicePage.currentPage - 1, props.servicePage.currentPage + 5).map((page) => {
                    return (
                      ...
                      <li onClick={() => {
                        ...
                        <a className="page-link" >
                          <p className={props.servicePage.currentPage === page ? "selected" : ""}>
                            {page}
                          </p>
                        </a>
                      </li>
                  ...
                  pages.slice(pagesCount - 6, pagesCount - 1).map((page) => {
                    return (
                      <li onClick={() => {
                        ...
                        <a className="page-link" >
                          <p
                            className={props.servicePage.currentPage === page ? "selected" : ""}>
                            {page}
                          </p>
                        </a>
                      </li>
                ...
                <li onClick={() => {
                  props.onChangePage({
                    pageNumber: props.servicePage.currentPage + (props.servicePage.currentPage + 1 <= pagesCount ? 1 : 0),
                    input: isSomeParams.isTextForSearch,
                    field: isSomeParams.isFieldForSearch
                  })
                }} className="page-item">
